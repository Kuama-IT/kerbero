...
public class NukiOAuthRepositoryTests : IDisposable
{
  ...
  public void BuildUriForCode_ArgumentNullException()
  {
    ...
    // Act
    var exCode = _nukiOAuthRepository.GetOAuthRedirectUri("INVALID_CLIENT_ID");
    // Assert
    exCode.IsFailed.Should().BeTrue();
    exCode.Errors.FirstOrDefault().Should().BeOfType<InvalidParametersError>();
    _configurationMock.Setup(m => m["NUKI_REDIRECT_FOR_TOKEN"]).Returns("/nuki/auth/token");
  ...
  public async void Authenticated_ButNukiReturnsInvalidParameterError()
  {
    ...
    // Act
    // Assert
    var ex = await _nukiOAuthRepository.Authenticate("clientId", "code");
