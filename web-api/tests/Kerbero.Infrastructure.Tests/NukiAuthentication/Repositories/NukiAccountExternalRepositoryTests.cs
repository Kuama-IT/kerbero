...
public class NukiAccountExternalRepositoryTests: IDisposable
{
	...
	public NukiAccountExternalRepositoryTests()
	{
		...
		_nukiOAuthRepository = new NukiOAuthRepository(_configurationMock.Object, nukiSafeHttpCallHelper);
	...
	{
		// Act
		var redirect = _nukiOAuthRepository.GetOAuthRedirectUri(new NukiAccountBuildUriForCodeExternalRequest("v7kn_NX7vQ7VjQdXFGK43g"));
	...
	{
		// Act
		var exCode = _nukiOAuthRepository.GetOAuthRedirectUri(new NukiAccountBuildUriForCodeExternalRequest(""));
	...
	public void BuildUriForCode_ArgumentNullException_Test()
	{
		...
		// Act
		var exCode = _nukiOAuthRepository.GetOAuthRedirectUri(new NukiAccountBuildUriForCodeExternalRequest("INVALID_CLIENT_ID"));
		// Assert
		exCode.IsFailed.Should().BeTrue();
		exCode.Errors.FirstOrDefault().Should().BeOfType<InvalidParametersError>();
		_configurationMock.Setup(m => m["NUKI_REDIRECT_FOR_TOKEN"]).Returns("/nuki/auth/token");
	...
	public async void GetNukiAccount_ReturnSuccess_Test()
	{
		...
		// Act
		var nukiAccount = await _nukiOAuthRepository.GetNukiAuthentication(new NukiOAuthRequest(){ ClientId = "clientId", Code = "code"});
	...
	{
		// Act
		var exToken = await _nukiOAuthRepository.GetNukiAuthentication(new NukiOAuthRequest() {ClientId = "", Code = ""}) ;
		// Assert
    ...
    public async void GetNukiAccount_ButNukiReturnsInvalidParameterError_Test()
    {
	    ...
	    // Act
	    // Assert
	    var ex = await _nukiOAuthRepository.GetNukiAuthentication(new NukiOAuthRequest() {ClientId = "clientId", Code = "code"});
	...
	public async void RefreshToken_ReturnSuccess_Test()
	{
		...
		// Act
		var nukiAccount = await _nukiOAuthRepository.RefreshToken(
	...
	{
		// Act
		var exToken = await _nukiOAuthRepository.RefreshToken(new NukiOAuthRequest() {ClientId = "", Code = ""}) ;
		// Assert
		exToken.IsFailed.Should().BeTrue();
		exToken.Errors.FirstOrDefault().Should().BeOfType<InvalidParametersError>();
	}
